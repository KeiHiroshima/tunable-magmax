...
from src.heads import build_subset_classification_head, get_classification_head


def get_dataset_and_classifier_for_split(
    dataset, split_idx, text_encoder, args, remap_labels=True, return_classifier=True
):
    if args.split_strategy == "data":
        ...
        dataset.train_dataset = torch.utils.data.Subset(
            dataset.train_dataset, train_subset_indices
        )
