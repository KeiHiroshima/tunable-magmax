# Standard dataset templates adapted from:
#   MagMax (Marczak et al., ECCV 2024) https://github.com/danielm1405/magmax
#
# The following functions (generate_composite_factors, compose_template) and
# cub200 compositional templates are taken from:
#   PerceptionCLIP (Yu et al., ICLR 2024) https://github.com/umd-huang-lab/perceptionCLIP
#   MIT License — Copyright (c) 2023 CMU Locus Lab
...
def compose_template(org_templates, factor_templates):
    ...
    for factors in factor_templates:
        ...
        for descriptions in factors:
            if descriptions:
                new_factors.append(
                    lambda c, main=org_templates[0], descriptions=descriptions: (
                        main(c) + ", " + descriptions + "."
                    )
                )
            else:
...
cub200_main_template = [lambda c: f"a photo of a {c}, a type of bird"]
...
cub200_template = compose_template(
    cub200_main_template, generate_composite_factors(cub200_factor_templates)
)
